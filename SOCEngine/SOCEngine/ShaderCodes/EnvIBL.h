...
float3 GetSkyLightReflection(float3 reflectDir, float roughness, bool isDynamicSkyLight)
{
	...
	float3 reflection		= ambientCubeMap.SampleLevel(skyCubeMapSampler, reflectDir, absSpecularMip).rgb;

	if(isDynamicSkyLight)
	{
		float3 LowFrequencyReflection = ambientCubeMap.SampleLevel(skyCubeMapSampler, reflectDir, maxMipCount).rgb;
...
float3 PreFilterEnvMap(float roughness, float3 reflectDir, uniform uint numSample)
{
	...
	for( uint i = 0; i < numSample; i++ )
	{
		...
		if( NdotL > 0 )
		{
			filteredColor += ambientCubeMap.SampleLevel( skyCubeMapSampler, lightDir, 0 ).rgb * NdotL;
			weight += NdotL;
		}
	}
...
//	NdotV		= saturate(NdotV - 0.0002f);
	...
	{
		prefilteredColor	= ambientCubeMap.SampleLevel(skyCubeMapSampler, reflectDir, mip).rgb;
...
float3 SpecularIBL(float3 specularColor, float roughness, float3 normal, float3 viewDir, uniform uint numSamples)
{
	...
	for( uint i = 0; i < numSamples; i++ )
	{
		...
		if( NdotL > 0 )
		{
			float3 SampleColor = ambientCubeMap.SampleLevel( skyCubeMapSampler, lightDir, 0 ).rgb;
...
float3 ApproximateIBL(Texture2D<float2> preIntegrateEnvMap, ApproximateIBLParam param)
{
	float3 diffuse = float3(0.0f, 0.0f, 0.0f);
	{
		...
		float3 diffsueLookUp = ambientCubeMap.SampleLevel(skyCubeMapSampler, param.normal, absDiffuseMip).rgb;
