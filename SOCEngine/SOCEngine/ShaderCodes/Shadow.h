...
SamplerComparisonState shadowSamplerGreaterCmpState	:	register( s2 );
SamplerState shadowSamplerState						:	register( s3 );
SamplerState pointSamplerState						:	register( s4 );
SamplerComparisonState shadowSamplerLessCmpState	:	register( s6 );
...
float PCF(Texture2D<float> atlas, SamplerComparisonState cmpSampler, float2 uv, float depth, float2 stepUV)
{
	float shadow = 0.0f;
	[unroll] for(int i=-SHADOW_KERNEL_LEVEL; i<=SHADOW_KERNEL_LEVEL; ++i)
	{
		[unroll] for(int j=-SHADOW_KERNEL_LEVEL; j<=SHADOW_KERNEL_LEVEL; ++j)
		{
			...
			shadow += (atlas.SampleCmpLevelZero(cmpSampler, uv + offset, depth).x);
		}
	}
...
/***********	PCSS	************/
float ComputePenumbraSize(float2 uv, float receiver, float softness, float2 rcpAtlasMapSize)
{
	...
	for(float x = -BLOCKER_SEARCH_STEP_COUNT; x <= BLOCKER_SEARCH_STEP_COUNT; ++x)
	{
		for(float y = -BLOCKER_SEARCH_STEP_COUNT; y <= BLOCKER_SEARCH_STEP_COUNT; ++y)
		{
			...
			float shadowMapDepth = DirectionalLightShadowViewDepthMap.SampleLevel(pointSamplerState, uv + offset, 0).x;
...
float PCSS(float2 uv, float viewDepth, float softness, float2 rcpAtlasMapSize)
{
	float penumbra		= ComputePenumbraSize(uv, viewDepth, softness, rcpAtlasMapSize);
	float2 stepUV		= penumbra * rcpAtlasMapSize;
	return PCF(DirectionalLightShadowViewDepthMap, shadowSamplerLessCmpState, uv, viewDepth, stepUV);
}
/***********	PCSS	************/
...
float4 RenderSpotLightShadow(uint lightIndex, float3 vertexWorldPos, float shadowDistanceTerm)
{
	...
	float shadow		= saturate( PCF(SpotLightShadowMapAtlas, shadowSamplerGreaterCmpState, shadowUV.xy, depth + bias, stepUV) );
...
float4 RenderDirectionalLightShadow(uint lightIndex, float3 vertexWorldPos, float3 lightDir)
{
	...
	float penumbra			= ComputePenumbraSize(shadowUV.xy, viewDepth, shadowParam.softness, rcpAtlasMapSize);
	float shadow			= saturate(PCSS(shadowUV.xy, viewDepth, shadowParam.softness, rcpAtlasMapSize));
#endif
...
float4 RenderPointLightShadow(uint lightIndex, float3 vertexWorldPos, float3 lightDir, float shadowDistanceTerm)
{
	...
	float shadow		= saturate( PCF(PointLightShadowMapAtlas, shadowSamplerGreaterCmpState, shadowUV.xy, depth + bias, stepUV) );
