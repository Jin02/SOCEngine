...
bool CalcDepthBoundMSAA(uint2 globalIdx)
{
	...
	{
		float opaqueDepth		= DepthBuffer.Load(uint2(globalIdx.x, globalIdx.y), sampleIdx).x;
		float opaqueDepthToView		= InvertProjDepthToView(opaqueDepth);
#if defined(ENABLE_BLEND)
		float blendedDepth		= BlendedDepthBuffer.Load(uint2(globalIdx.x, globalIdx.y), sampleIdx).x;
...
{
	float opaqueDepth		= DepthBuffer.Load( uint3(globalIdx.x, globalIdx.y, 0) ).x;
	float opaqueDepthToView		= InvertProjDepthToView(opaqueDepth);
	uint opaqueZ			= asuint(opaqueDepthToView);

#if defined(ENABLE_BLEND)
	float blendedDepth		= BlendedDepthBuffer.Load( uint3(globalIdx.x, globalIdx.y, 0) ).x;
	float blendedDepthToView	= InvertProjDepthToView(blendedDepth);
