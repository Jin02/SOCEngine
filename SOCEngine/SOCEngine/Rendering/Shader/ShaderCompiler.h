...
namespace Rendering
{
	namespace Shader
	{
		...
		public:
			...
			static DXResource<ID3DBlob>	Compile(const std::string &fileFullPath, const std::string& shaderCode, const std::string& shaderModel, const std::string& funcName, const std::vector<Shader::ShaderMacro>* macros);
			static std::string			MakeShaderFileFullPath(const std::string& folderPath, const std::string& fileName);

			DXResource<ID3DBlob>		CreateBlob(const std::string& folderPath,
