...
void AnisotropicVoxelMapAtlas::Initialize(
	uint sideLength, uint maxNumOfCascade, DXGI_FORMAT format, uint mipmapLevels)
{
...
void AnisotropicVoxelMapAtlas::BindUAVsToPixelShader(const Device::DirectX* dx, const uint bindSlotOffset)
{
	ID3D11DeviceContext* context	= dx->GetContext();
	const uint initCounts[1]		= {0xFFFFFFFFu};		
	ID3D11UnorderedAccessView* view	= _voxelMapAtlas->GetUnorderedAccessView()->GetView();

	context->OMSetRenderTargetsAndUnorderedAccessViews(D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL, nullptr, nullptr, 0, 1, &view, initCounts);
}

void AnisotropicVoxelMapAtlas::UnbindUAVs(const Device::DirectX* dx, const uint bindSlotOffset)
{
	ID3D11DeviceContext* context = dx->GetContext();
	const uint initCounts[1] = {0xFFFFFFFFu};		
