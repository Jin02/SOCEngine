...
void LightManager::InitializeAllShaderResourceBuffer()
{
	...
	// Point Light
	{
		...
		_pointLightTransformSRBuffer->Initialize(
			sizeof(BaseLight::LightTransformBuffer), POINT_LIGHT_BUFFER_MAX_NUM,
	...
	// Spot Light
	{
		...
		_spotLightTransformSRBuffer->Initialize(
			sizeof(BaseLight::LightTransformBuffer), SPOT_LIGHT_BUFFER_MAX_NUM,
			DXGI_FORMAT_R32G32B32A32_FLOAT,
			dummyData, true, 0, D3D11_USAGE_DYNAMIC);
...
uint LightManager::Add(BaseLight*& light)
{
...
bool UpdateBuffer(ID3D11DeviceContext* context,
				  ...
				  std::function<ushort(const BaseLight*)> getShadowIndexInEachShadowLights,
...
void LightManager::UpdateSRBufferUsingMapDiscard(ID3D11DeviceContext* context,
												 const std::function<ushort(const BaseLight*)>& getShadowIndexInEachShadowLights,
												 const std::function<uchar(const Light::BaseLight*)>& getLightShaftIndex)
{
	...
	{
		const Light::BaseLight* light	= iter->light;
		BaseLight::LightType lightType	= light->GetType();
		...
		else if(lightType == BaseLight::LightType::Point)
		{	
			isUpdatedPL = UpdateBuffer<PointLight, PointLight::Param, BaseLight::LightTransformBuffer>(
				context, _lights, allLightBufferIdx,
		...
		else if(lightType == BaseLight::LightType::Spot)
		{
			isUpdatedSL = UpdateBuffer<SpotLight, SpotLight::Param, BaseLight::LightTransformBuffer>(
				context, _lights, allLightBufferIdx,
	...
	if(isUpdatedPL || _forceUpdatePL)
	{
		...
		_pointLightTransformSRBuffer->UpdateResourceUsingMapUnMap(context, data, count * sizeof(BaseLight::LightTransformBuffer));
	...
	if(isUpdatedSL || _forceUpdateSL)
	{
		...
		_spotLightTransformSRBuffer->UpdateResourceUsingMapUnMap(context, data, count * sizeof(BaseLight::LightTransformBuffer));
...
void LightManager::Delete(const BaseLight*& inputLight)
{
	address key = reinterpret_cast<address>(inputLight);

	const Light::BaseLight* light = _lights.Find(key)->light;
	Light::BaseLight::LightType type = light->GetType();
	...
	else if(type == BaseLight::LightType::Point)
	{
	...
	else if(type == BaseLight::LightType::Spot)
	{
...
bool LightManager::Has(BaseLight*& light) const
{
	address key = reinterpret_cast<address>(light);
	return _lights.Has(key);
}

uint LightManager::FetchLightIndexInEachLights(const BaseLight* inputLight) const
{
	uint index = -1;
	...
	if(inputLight)
	{
		...
		if(type == BaseLight::LightType::Directional)	_directionalLights.Find(key,	&index);
		else if(type == BaseLight::LightType::Point)	_pointLights.Find(key,			&index);
		else if(type == BaseLight::LightType::Spot)		_spotLights.Find(key,			&index);
	}
