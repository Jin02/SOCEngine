...
namespace Rendering
{
	namespace Geometry
	{
		...
		public:
			...
			PtrType&		Get(uint index)			{ return _meshes[index]; }
			const PtrType&	Get(uint index)	const	{ return _meshes[index]; }

			const PtrType* Find(Core::ObjectID::LiteralType id) const;
			PtrType* Find(Core::ObjectID::LiteralType id)
			{
				return const_cast<PtrType*>( static_cast<const MeshRawPtrs*>(this)->Find(id) );
			}
