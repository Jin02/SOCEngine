...
namespace readers {
	...
	public:
		...
		const char *placeHolderMaterialName;
		...
		FbxConverter(fbxconv::log::Log *log, TextureInfoCallback textureCallback) 
			:	log(log), scene(0), textureCallback(textureCallback), placeHolderMaterialName("FBXCONV_PLACEHOLDER_MATERIAL") {
		...
		void updateNode(Model * const &model, Node * const &node) {
			...
			if (fbxMeshMap.find(node->source->GetGeometry()) != fbxMeshMap.end()) {
				...
				if (parts.size() > 0u && matCount < parts.size())
				...
				for (unsigned int i = 0; i < parts.size(); i++) {
					...
					for (unsigned int j = 0; j < parts[i].size(); j++) {
						if (parts[i][j]) {
							...
							for (unsigned int k = 0; k < nodePart->meshPart->sourceBones.size(); k++) {
		...
		void addMesh(Model * const &model, FbxMeshInfo * const &meshInfo, FbxNode * const &node) {
			...
			for (int i = 0; i < meshInfo->meshPartCount; i++) {
				...
				for (int j = 0; j < m; j++) {
					...
					if (j < n)
						for (unsigned int k = 0; k < meshInfo->partBones[i][j].size(); k++)
